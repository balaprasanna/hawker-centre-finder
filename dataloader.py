...
class ProcessGeoJsonFile:
    ...
    def extract_data_from_geo_json_file(self):
        ...
        zone_levels = [20, 19, 17, 16, 15, 14, 13, 12, 11, 10]
        zone_cols = [f"ZONE_{zo}" for zo in zone_levels]
    ...
    def convert_lang_long_to_s2cells(s, levels):
        return pd.Series({f"ZONE_{level}": s2cell.lat_lon_to_cell_id(s.LAT, s.LONG, level=level) for level in levels})
